    ...
    ajaxUplaodFile: function (file, postData, events) {
        ...
        if (file.size <= partSize) {
            ...
            for (var i = 0, index = 1; i < file.size; index++, i += partSize) {
                var _part = {};
                _part.getFile = (function(start,end,info){
                    return function() {
                        return new Blob([file.slice(start, end)], info);
                    }
                })(i,Fantasy.getMin(file.size, i + partSize),{type:'application/octet-stream',name:file.name + '.part' + ('' + index).addZeroLeft(10),lastModified: new Date()});
                _part.index = index;
            ...
            var _events = {
                success: function (data) {
                    if (!!fileParts.length) {
                        ...
                        _xhr = zhis.ajaxUplaodFile(part.getFile(), getPostData(part), _events);
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            if (data.hasOwnProperty('fileDetail')) {//如果上传过此文件，直接忽略本次上传
                                ...
                                fileParts.each(function (i, v) {
                                    new hashMe(v.getFile(), function (hash) {
                                        if (!v.partFileHash) {
                                            v.partFileHash = hash;
