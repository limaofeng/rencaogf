Fantasy.apply(Date, {
    ...
    compareTime:function(){
        var compare=(new Date().valueOf()-this.valueOf())/1000,cf=function(){
            compare=compare/60;
            return compare;
        }
        if(compare<1)return "刚刚";
        if(compare<60)return parseInt(compare)+"秒前";
        if(cf()<60)return parseInt(compare)+"分钟前";
        if(cf()<12)return this.format("HH:mm:ss");
        compare=compare/24;
        if(compare<8) return compare<1?1:parseInt(compare)+"天前";
    ...
    return {
        ...
        parse: function(source, formatter){
            try {
                ...
                var data = [];
                var formatters = ['yyyy', 'MM', 'dd', 'HH', 'mm', 'ss'];
