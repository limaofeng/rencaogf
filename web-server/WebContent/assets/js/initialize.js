    ...
    */
    ...
    initializes.push(function () {
        $('.img-rounded,.img-circle,.img-thumbnail,img[data-src^="holder.js"]', this).filter(function(){
            ...
            if(!!$(zhis).attr('src')){
                var execs = new RegExp("\/([0-9]+)x([0-9]+)\/").exec($(zhis).data('src'));
                if(!!execs){
                    $(zhis).css({width:execs[1],height:execs[2]})
                }
                var image = new Image();
    ...
    Fantasy.Ajax.on('error', function (xhr, status, err, errorHeader, s) {
        if ((status == 200 || status == 403) && (err == 'struts2 action Error' || err == 'Forbidden')) {
            var errorMsg = '';
            if (!!s.form && s.form.length > -1) {
                s.form.find('[id$=Tip]').removeClass('onError').html("");
            }
            if (errorHeader.errorMessages && errorHeader.errorMessages.length != 0) {
            ...
            }else if (xhr.responseJSON.length != 0) {
                    top.$.msgbox({
                        msg: xhr.responseJSON.errorMessages[0],
                        icon: "error"
                    });
            } else {
