...
<script type="text/javascript">
    $(function () {
        var image = <@s.property value="@com.fantasy.framework.util.jackson.JSON@serialize(art.logo)" escapeHtml="false" default="[]"/>;
        ...
        $("#saveForm").ajaxForm({
            beforeSerialize : function(zhis, options){
                ...
                if(_images.length > 0){
                    _data['logo'] = _images[0].fileManagerId + ':' + _images[0].absolutePath;
                }
                options.data = _data;
            },
